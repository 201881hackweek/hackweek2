...
public class Prop8 : MonoBehaviour
{
    //背包栏的8dirtykey
    private Vector2 origin;
    private PropEventManager propeventmanager;
    private float Propwidth = 0.5f;//测试用//后续调
    public RectTransform Prop9_little;//与pencildust叠加出的光滑钥匙
    ...
    void OnMouseUp()
    {
        ...
        if (hit && hit.collider.gameObject.tag == "pencildust")
        {
            ...
            RectTransform prop9 = Instantiate(Prop9_little, new Vector3(), new Quaternion(0, 0, 0, 0))as RectTransform;//在背包栏生成光滑钥匙，位置位于pencildust
            prop9.SetParent(hit.collider.transform.parent);
            prop9.anchoredPosition = new Vector2(50, -50);
            prop9.transform.localScale = new Vector3(1, 1, 1);
