...
public class Prop18 : MonoBehaviour
{
    //背包栏的18爸爸的碎片
    ...
    void OnMouseUp()
    {
        ...
        if (hit && hit.collider.gameObject.tag == "bathtub")//拖到浴缸烧碎片
        {
            ...
            if (propeventmanager.bathtubnumber != 2)//拖的不是最后一个 这才弹出信息框 否则要弹出总的信息（爸爸碎片烧了之类）
            {
                propeventmanager.propnumber = 18;
                propeventmanager.bathtubnumber++;//使用打火机后弹出的信息（可无）
            }
            else propeventmanager.propnumber = 25;//放最后吧..
            for (int i = 0; i < 16; i++)
            {
