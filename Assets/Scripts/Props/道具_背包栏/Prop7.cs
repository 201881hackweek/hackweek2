...
public class Prop7 : MonoBehaviour
{
    ...
    public RectTransform Prop8_little;//梳头后获得的8钥匙
    ...
    void OnMouseUp()
    {
        ...
        if (hit && hit.collider.gameObject.tag == "hair")
        {
            propeventmanager.Dialogbox.SetActive(true);
            propeventmanager.propnumber = 7;//用道具编号了
            for (int i = 0; i < 16; i++)
            {
            ...
            RectTransform prop8 = Instantiate(Prop8_little, new Vector3(), new Quaternion(0, 0, 0, 0));//在背包栏生成8钥匙
            propeventmanager.Updategridposition(prop8);
