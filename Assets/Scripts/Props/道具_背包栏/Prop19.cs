...
public class Prop19 : MonoBehaviour
{
    //背包栏的19爸爸房间钥匙
    ...
    void OnMouseUp()
    {
        ...
        if (hit && hit.collider.gameObject.tag == "prop19belonging")//爸爸房间门
        {
            propeventmanager.Dialogbox.SetActive(true);
            propeventmanager.propnumber = 19;//使用爸爸房间钥匙后弹出的信息框
            hit.collider.GetComponent<Door>().hasOpended = true;
            /////////加爸爸房间的has opened true//一定记得！！！！！！
            for (int i = 0; i < 16; i++)
            {
